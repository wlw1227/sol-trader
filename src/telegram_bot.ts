...
import { computePositionStats } from "./trader";
...
async function getSolBalance(): Promise<number> {
  ...
  const wallet = Keypair.fromSecretKey(bs58.decode(PRIV));
...
async function handleKillCommand() {
  if (isKillEngaged()) {
    await sendTelegramMessage("☠️ Kill switch is *already* engaged.");
    return;
  }

  engageKillSwitch();
  await sendTelegramMessage(
    "☠️ *Kill switch engaged.*\nAll trading processes that check the kill flag will exit shortly."
  );
}

async function handleResumeCommand() {
  if (!isKillEngaged()) {
    await sendTelegramMessage("ℹ️ Kill switch is already *OFF*.");
    return;
  }
